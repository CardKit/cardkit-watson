...
public class WatsonVisualRecognitionToken: ExecutableToken {
    ...
    public func classify(image: UIImage, threshold: Double?) throws -> [WCKDetectedObject] {
    ...
    public func classify(imagePath: String, threshold: Double?) throws -> [WCKDetectedObject] {
        let url = URL(fileURLWithPath: imagePath)
        return try self.classify(imagePath: url, threshold: threshold)
    }
    
    public func classify(imagePath: URL, threshold: Double?) throws -> [WCKDetectedObject] {
                ...
                for image in classifiedImages.images {
                    for classifier in image.classifiers {
                        for cls in classifier.classes {
                            let object = WCKDetectedObject(objectName: cls.classification, confidence: cls.score)
